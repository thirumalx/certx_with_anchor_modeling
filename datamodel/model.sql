...
--
-- Anchor table -------------------------------------------------------------------------------------------------------
-- US_User table (with 3 attributes)
-----------------------------------------------------------------------------------------------------------------------
...
-- Historized attribute table -----------------------------------------------------------------------------------------
-- US_PWD_User_Password table (on US_User)
-----------------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS certx.US_PWD_User_Password (
    US_PWD_US_ID bigint not null,
    US_PWD_User_Password varchar(250) not null,
    US_PWD_ChangedAt timestamp not null,
    Metadata_US_PWD bigint not null,
    constraint fkUS_PWD_User_Password foreign key (
        US_PWD_US_ID
    ) references certx.US_User (US_ID),
    constraint pkUS_PWD_User_Password primary key (
        US_PWD_US_ID ,
        US_PWD_ChangedAt
    ) include (
        US_PWD_User_Password
    )
);
-- Historized attribute table -----------------------------------------------------------------------------------------
...
-- Anchor table -------------------------------------------------------------------------------------------------------
-- AP_Application table (with 1 attributes)
-----------------------------------------------------------------------------------------------------------------------
...
-- Anchor table -------------------------------------------------------------------------------------------------------
-- CE_Certificate table (with 5 attributes)
-----------------------------------------------------------------------------------------------------------------------
...
;
-- Restatement Finder Function and Constraint -------------------------------------------------------------------------
-- rfUS_PWD_User_Password restatement finder, also used by the insert and update triggers for idempotent attributes
-- rcUS_PWD_User_Password restatement constraint (available only in attributes that cannot have restatements)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION certx.rfUS_PWD_User_Password(
    id bigint,
    value varchar(250),
    changed timestamp
) RETURNS smallint AS '
    BEGIN
        IF EXISTS (
            ...
            WHERE
                value = (
                    SELECT
                        pre.US_PWD_User_Password
                    FROM
                        certx.US_PWD_User_Password pre
                    WHERE
                        pre.US_PWD_US_ID = id
                    AND
                        pre.US_PWD_ChangedAt < changed
                    ORDER BY
                        pre.US_PWD_ChangedAt DESC
                    LIMIT 1
            )
        )
        OR EXISTS(
            ...
            WHERE
                value = (
                    SELECT
                        fol.US_PWD_User_Password
                    FROM
                        certx.US_PWD_User_Password fol
                    WHERE
                        fol.US_PWD_US_ID = id
                    AND
                        fol.US_PWD_ChangedAt > changed
                    ORDER BY
                        fol.US_PWD_ChangedAt ASC
                    LIMIT 1
            )
        )
        THEN
...
;
-- Latest perspective -------------------------------------------------------------------------------------------------
-- lUS_PWD_User_Password viewed by the latest available information (may include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW certx.lUS_PWD_User_Password AS
SELECT DISTINCT ON (US_PWD_US_ID) US_PWD_US_ID
     , US_PWD_User_Password
     , US_PWD_ChangedAt
     , Metadata_US_PWD
  FROM certx.US_PWD_User_Password
 ORDER 
    BY US_PWD_US_ID DESC
     , US_PWD_ChangedAt DESC 
;
-- Attribute Point-in-time perspective -------------------------------------------------------------------------------------------------
-- pUS_PWD_User_Password viewed as it was on the given timepoint
-----------------------------------------------------------------------------------------------------------------------
 CREATE OR REPLACE FUNCTION certx.pUS_PWD_User_Password
      ( changingTimepoint timestamp
      ) 
RETURNS TABLE
      ( US_PWD_US_ID bigint
      , US_PWD_User_Password varchar(250)
      , US_PWD_ChangedAt timestamp 
      , Metadata_US_PWD bigint
      ) 
AS 
'
 SELECT DISTINCT ON (US_PWD_US_ID) US_PWD_US_ID
      ...
      , Metadata_US_PWD
   FROM certx.US_PWD_User_Password
  WHERE US_PWD_ChangedAt <= changingTimepoint
  ORDER 
     BY US_PWD_US_ID DESC
      , US_PWD_ChangedAt DESC 
 ;
...
;
-- Now perspective ----------------------------------------------------------------------------------------------------
-- nUS_PWD_User_Password viewed as it currently is (cannot include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW certx.nUS_PWD_User_Password AS
SELECT *
  FROM certx.pUS_PWD_User_Password(now()::timestamp)
;
-- Latest perspective -------------------------------------------------------------------------------------------------
...
SELECT US.US_ID
     ...
     , PWD.US_PWD_US_ID
     , PWD.Metadata_US_PWD
     , PWD.US_PWD_ChangedAt
     , PWD.US_PWD_User_Password
     , NAM.US_NAM_US_ID
     , NAM.Metadata_US_NAM
  ...
  LEFT 
  JOIN certx.lUS_PWD_User_Password PWD
    ON PWD.US_PWD_US_ID = US.US_ID
  LEFT 
...
'
 SELECT US.US_ID
   ...
   JOIN certx.pUS_UID_User_UserId(changingTimepoint) UID
     ON UID.US_UID_US_ID = US.US_ID
   LEFT 
   JOIN certx.pUS_PWD_User_Password(changingTimepoint) PWD
     ON PWD.US_PWD_US_ID = US.US_ID
   LEFT 
 ...
 SELECT timepoints.inspectedTimepoint
   ...
   FROM (
          SELECT DISTINCT US_UID_US_ID AS US_ID
           ...
           UNION
          SELECT DISTINCT US_PWD_US_ID AS US_ID
               , US_PWD_ChangedAt::timestamp inspectedTimepoint
               , ''PWD'' AS mnemonic
            FROM certx.US_PWD_User_Password
           WHERE (selection is null OR selection like ''%PWD%'')
             AND US_PWD_ChangedAt BETWEEN intervalStart AND intervalEnd
           UNION
...
SELECT AP.AP_ID
  ...
  LEFT 
  JOIN certx.AP_NAM_Application_Name NAM
